/**
 * PGlite database client for Web mode
 *
 * Web-compatible version of the database client using IndexedDB persistence
 */
...
export class WebPGliteDB {
  private db: PGlite | null = null
  private dbUrl: string

  constructor(dbUrl: string = 'idb://brio-db') {
    this.dbUrl = dbUrl
  }
  ...
  async createNote(title: string, slug: string, content: string | null): Promise<string> {
    ...
    while (true) {
      try {
        await this.db.query('INSERT INTO notes (id, title, slug, content, type) VALUES ($1, $2, $3, $4, $5)', [
          id,
          title,
          finalSlug,
          content,
  ...
  getDatabaseSize(): number {
    // For web mode with IndexedDB, we cannot easily get the size
    // Return 0 as a placeholder (could be implemented with IndexedDB API if needed)
    return 0
  }
  ...
  async updateLinksOnRename(oldTitle: string, newTitle: string): Promise<void> {
    ...
    for (const row of notesWithLinks.rows) {
      const noteId = (row as { from_note_id: string }).from_note_id
  ...
  async searchNotes(query: string): Promise<Array<Note & { preview?: string }>> {
    ...
    const words = normalizedQuery.split(/\s+/).map((w) => this.removeAccents(w))
  ...
  async getNotesByTag(tag: string): Promise<string[]> {
    ...
    return result.rows.map((row) => (row as { note_id: string }).note_id)
  }
   ...
   * Supports: - [ ], - [x], - [>], - [-]
   */
  private parseTasks(content: string): Array<{ content: string; status: TaskStatus; line_number: number }> {
    ...
    for (let i = 0; i < lines.length; i++) {
      ...
      if (match) {
        ...
        console.log('[parseTasks] Line:', JSON.stringify(line))
  ...
  async saveQuickCapture(content: string): Promise<void> {
    ...
    let inboxNote = await this.db.query('SELECT * FROM notes WHERE title = $1 AND deleted_at IS NULL', [
      'Inbox',
    ])
    ...
    await this.db.query('UPDATE notes SET content = $1, updated_at = CURRENT_TIMESTAMP WHERE id = $2', [
      newContent,
      inboxId,
    ])
  }
